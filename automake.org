...
evolved than the =POSIX= standard, so =Makefile=s designed for GNU Make are not
...
- Give up writing your =Makefiles=. Write *scripts* (Bash, Python) to write them for you.
