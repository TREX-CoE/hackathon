...
* More about maintainer mode

** Automatic maintainer/user switching
...
AC_CHECK_FILE([.git], [IS_MAINTAINER="yes"], [IS_MAINTAINER="no"])
   ...
   =configure.ac= by checking the value of this variable:
...
** Example: Saving the Git Hash

   Inside =configure.ac=, it is rather simple to get the git hash and
...
AC_DEFINE_UNQUOTED(GIT_HASH, ["${GIT_HASH}"], [Git SHA1 Hash])
   ...
   =.git_hash=. In user mode, we read the content of =.git_hash=.
   Then, we propagate this variable to =config.h= with
   ~AC_DEFINE_UNQUOTED~.
...
* Useful links
  ...
  - [[https://www.gnu.org/software/autoconf-archive/The-Macros.html][Autoconf archive of macros]]
  ...
  - [[https://aoterodelaroza.github.io/devnotes/modern-fortran-makefiles/][Modern Fortran Makefiles]]: talks about the problems due to using
    modules, and proposes a solution for Automatic dependency generation
  - [[https://gitlab.mpcdf.mpg.de/elpa/elpa/-/tree/master/fdep][fdep]] : Small set of scripts to teach autoconf/automake (using GNU
    make) about the additional dependencies in Fortran 90 files due to
    modules. 
