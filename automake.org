...
AM_INIT_AUTOMAKE([foreign subdir-objects silent-rules])
  ...
  The ~slient-rules~ option causes *Automake* to generate Makefiles
...
AM_CPPFLAGS =  -I$(srcdir)/include/
  ...
  ~srcdir~ is ~.~ but for /out of source/ builts it is set to the
...
* Configuration headers
  ...
  preprocessor variables. Inserting the following in =configure.ac=:
